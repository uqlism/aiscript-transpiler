...
import { convertArrayAssignment, convertObjectAssignment } from "../../utils/destructuring"
...
export class BinaryExpressionPlugin extends TranspilerPlugin {
    ...
    override tryConvertExpressionAsStatements = (node: ts.Expression) => {
        ...
        if (ts.isBinaryExpression(unwrapped)) {
            // 分割代入の場合
            if (unwrapped.operatorToken.kind === ts.SyntaxKind.EqualsToken && (ts.isArrayLiteralExpression(unwrapped.left) || ts.isObjectLiteralExpression(unwrapped.left))) {
                return this.convertDestructuringAssignment(unwrapped)
            } else if ([ts.SyntaxKind.EqualsToken, ts.SyntaxKind.PlusEqualsToken, ts.SyntaxKind.MinusEqualsToken].includes(unwrapped.operatorToken.kind)) {
                // 通常の代入
    ...
    private convertDestructuringAssignment(node: ts.BinaryExpression): Ast.Statement[] {
    ...
    private convertBinaryAssignExpression(node: ts.BinaryExpression): Ast.Statement[] {
        ...
        switch (node.operatorToken.kind) {
            ...
            case ts.SyntaxKind.ExclamationEqualsToken:
                return [{ type: "assign", dest: left, expr: { type: "add", left: left, right, loc: dummyLoc }, loc: dummyLoc }];
            case ts.SyntaxKind.AsteriskEqualsToken:
                return [{ type: "assign", dest: left, expr: { type: "mul", left: left, right, loc: dummyLoc }, loc: dummyLoc }];
            case ts.SyntaxKind.SlashEqualsToken:
                return [{ type: "assign", dest: left, expr: { type: "div", left: left, right, loc: dummyLoc }, loc: dummyLoc }];
            case ts.SyntaxKind.PercentEqualsToken:
                return [{ type: "assign", dest: left, expr: { type: "rem", left: left, right, loc: dummyLoc }, loc: dummyLoc }];
            case ts.SyntaxKind.AsteriskAsteriskEqualsToken:
                return [{ type: "assign", dest: left, expr: { type: "pow", left: left, right, loc: dummyLoc }, loc: dummyLoc }];
            case ts.SyntaxKind.AmpersandAmpersandEqualsToken:
                return [{ type: "assign", dest: left, expr: { type: "and", left: left, right, loc: dummyLoc }, loc: dummyLoc }];
            case ts.SyntaxKind.BarBarEqualsToken:
                return [{ type: "assign", dest: left, expr: { type: "or", left: left, right, loc: dummyLoc }, loc: dummyLoc }];
            default:
