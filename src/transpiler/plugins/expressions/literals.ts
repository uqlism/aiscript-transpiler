...
export class LiteralPlugin extends TranspilerPlugin {
    ...
    private convertNoSubstitutionTemplateLiteral(node: ts.NoSubstitutionTemplateLiteral): Ast.Tmpl {
        return { type: "tmpl", tmpl: [{ type: "str", value: node.text, loc: dummyLoc }], loc: dummyLoc };
    }

    private convertArrayLiteralExpression(node: ts.ArrayLiteralExpression): Ast.Arr {
    ...
    private convertObjectLiteralExpression(node: ts.ObjectLiteralExpression): Ast.Obj {
    ...
    private convertMethodToInlineFunction(node: ts.MethodDeclaration): Ast.Fn {
        const params: { dest: Ast.Identifier; optional: boolean; default?: Ast.Expression }[] = [];
        const destructuringStatements: Ast.Statement[] = [];
        let paramIndex = 0;

        for (const param of node.parameters) {
            const isOptional = !!param.questionToken;
            const defaultValue = param.initializer ? this.converter.convertExpressionAsExpression(param.initializer) : undefined;
