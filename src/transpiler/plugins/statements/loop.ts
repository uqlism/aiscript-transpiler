...
export class LoopStatementsPlugin extends TranspilerPlugin {
    ...
    private convertStatementToStatements(node: ts.Statement): (Ast.Expression | Ast.Statement)[] {
        if (ts.isBlock(node)) {
            // ブロック文の場合は中身を展開
    ...
    private convertForStatement(node: ts.ForStatement): Ast.Block | Ast.Loop {
            ...
            if (ts.isVariableDeclarationList(node.initializer)) {
                ...
                for (const declaration of node.initializer.declarations) {
                    ...
                    if (ts.isIdentifier(nameNode)) {
                        ...
                        this.converter.throwError("for文では分割代入は現在サポートされていません", nameNode as ts.Node);
                    }
                }
            } else {
        ...
        return evalBody.length == 1 ? loopStatement : { type: 'block', statements: evalBody, loc: dummyLoc }
    }
    ...
    private convertWhileStatement(node: ts.WhileStatement): Ast.Loop {
        ...
        const body = node.statement ? this.convertStatementToStatements(node.statement) : [];
        ...
        if (condition.type === "bool" && condition.value == true) {
            ...
            const ifStatement: Ast.If = { type: 'if', cond: { type: 'not', expr: condition, loc: dummyLoc }, then: { type: 'break', loc: dummyLoc }, elseif: [], loc: dummyLoc };
            loopBody.push(ifStatement);
        }
