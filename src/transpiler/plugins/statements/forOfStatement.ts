...
export class ForOfStatementPlugin extends TranspilerPlugin {
    ...
    private convertForOfStatement(node: ts.ForOfStatement): Ast.Each {
        ...
        const allStatements = [...destructuringStatements, ...flattenedBodyStatements];
        const forBody: Ast.Statement | Ast.Expression = allStatements.length === 1 && allStatements[0] ? allStatements[0] : { type: "block", statements: allStatements, loc: dummyLoc };
