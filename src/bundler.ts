...
export class AiScriptBundler {
	...
	private resolveNameConflicts(): void {
		...
		for (const [_filePath, moduleInfo] of this.modules) {
			// まずエクスポートシンボルを処理
			for (const exportInfo of moduleInfo.exports) {
				this.renamer.generateUniqueName(
					exportInfo.originalName,
					exportInfo.symbol,
				);
			}
		...
		try {
			...
			if (error instanceof TranspilerError) {
				...
				throw new BundlerError(
					`Error transpiling bundled code: ${error.message}`,
					"bundled",
					error.getPosition().startLine,
					error.getPosition().startColumn,
					undefined,
					error,
				);
			} else {
				throw new BundlerError(
					`Error transpiling bundled code: ${error instanceof Error ? error.message : String(error)}`,
					"bundled",
					1,
					1,
					undefined,
	 ...
	 */
	private combineAllStatements(): string {
	...
	private getProcessingOrder(): string[] {
		...
		const visit = (filePath: string): void => {
			...
			// 依存先を先に処理
			for (const dependency of moduleInfo.dependencies) {
				const resolvedPath = this.resolvedPaths.get(
					`${filePath}:${dependency}`,
				);
				...
				if (symbol) {
					...
					if (newName && newName !== node.text) {
						...
						const regex = new RegExp(
							`\\b${this.escapeRegex(node.text)}\\b`,
							"g",
						);
