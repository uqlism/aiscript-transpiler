...
export class BinaryExpressionPlugin extends TranspilerPlugin {
        ...
        switch (node.operatorToken.kind) {
            ...
            case ts.SyntaxKind.PlusEqualsToken: {
                ...
                if (this.converter.doTypeCheck) {
                    ...
                    if (isStringLike(node.left, this.converter.typeChecker) ||
    ...
    convertLogicalOr(node) {
        ...
        if (isBooleanLikeType(typeChecker.getTypeAtLocation(node.left), typeChecker) &&
     ...
     *   a >= b → Str:lt(a, b) >= 0
     */
    buildStringCompare(left, right, op) {
        const strLtCall = {
            type: "call",
        ...
        const zero = { type: "num", value: 0, loc: dummyLoc };
        ...
        switch (node.operatorToken.kind) {
            case ts.SyntaxKind.PlusToken: {
                ...
                if (this.converter.doTypeCheck) {
                    ...
                    // 判定1: 結果型が string
                    if (!(resultType.flags & (ts.TypeFlags.Any | ts.TypeFlags.Unknown)) &&
                        typeChecker.isTypeAssignableTo(resultType, typeChecker.getStringType())) {
                        return { type: "tmpl", tmpl: [left, right], loc: dummyLoc };
                    }
                    // 判定2: オペランドに文字列成分 (union type 対応)
                    if (hasStringComponent(typeChecker.getTypeAtLocation(node.left)) ||
                        hasStringComponent(typeChecker.getTypeAtLocation(node.right))) {
                        return { type: "tmpl", tmpl: [left, right], loc: dummyLoc };
                    }
                }
                // 数値加算の型チェック
                validateNumberLike(node.left, this.converter, `算術演算子 '+' の左オペランドはNumber型またはString型である必要があります`);
                validateNumberLike(node.right, this.converter, `算術演算子 '+' の右オペランドはNumber型またはString型である必要があります`);
