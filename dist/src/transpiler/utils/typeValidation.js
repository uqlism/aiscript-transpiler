...
export function validateElementAccess(targetExpr, indexExpr, typeChecker) {
    ...
    if (targetType.flags & ts.TypeFlags.Union) {
        const unionType = targetType;
        const allArray = unionType.types.every((type) => typeChecker.isArrayType(type) || typeChecker.isTupleType?.(type));
        if (allArray) {
    ...
    if (isComplexType(targetTypeString) ||
