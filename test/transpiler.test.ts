...
const testcases: TestCase[] = [
	...
	{
		title: "インデックスアクセス - 配列",
		ts: `const arr = [1, 2, 3]; arr[10];`,
		ais: `let arr = [1, 2, 3]; arr[10];`,
	},
	{
		title: "インデックスアクセス - オブジェクト",
		ts: `const obj = {hello: "world"}; obj["hello"];`,
	...
	{
		title: 'if文の条件が文字列 (自動変換: != "")',
		ts: `if ("hello") { }`,
		ais: `if ("hello" != "") { }`,
	},
	{
		title: "if文の条件が数値 (自動変換: != 0)",
		ts: `if (42) { }`,
		ais: `if (42 != 0) { }`,
	},
	{
		title: "while文の条件が文字列 (自動変換)",
		ts: `while ("hello") { }`,
		ais: `loop { if !("hello" != "") break; }`,
	},
	{
		title: "for文の条件が数値 (自動変換)",
		ts: `for (let i = 0; 42; i++) { }`,
		ais: `eval { var i = 0; loop { if !(42 != 0) break; i += 1 } }`,
	},
	{
		title: "do-while文の条件が文字列 (自動変換)",
		ts: `do { } while ("test")`,
		valid: true,
	},
	{
		title: "else if文の条件が数値 (自動変換)",
		ts: `if (true) { } else if (123) { }`,
		ais: `if true { } elif (123 != 0) { }`,
	},
	{
	...
	{
		title: "三項演算子の条件が文字列 (自動変換)",
		ts: `const x = "hello" ? 1 : 2`,
		ais: `let x = if ("hello" != "") 1 else 2`,
	},
	{
		title: "三項演算子の条件が数値 (自動変換)",
		ts: `const x = 42 ? 1 : 2`,
		ais: `let x = if (42 != 0) 1 else 2`,
	},
	{
		title: "ネストした三項演算子の条件が文字列 (自動変換)",
		ts: `const x = true ? 1 : "test" ? 2 : 3`,
		ais: `let x = if true 1 elif ("test" != "") 2 else 3`,
	},
	{
		title: "正常な三項演算子",
		ts: `const x = true ? 100 : 200`,
	...
	{
		title: "論理否定演算子に数値 (自動変換: != 0)",
		ts: `let num: number = 42; !num`,
		ais: `var num = 42; !(num != 0)`,
	},
	{
